...
{
	struct ch36x_ioctl_data ioc_data = {.fd = fd, .offset = offset, .buf = ibuffer, .size = ilen};
	return ioctl(fd, CH36x_READ_PACKET, ioc_data);
}

extern int ch36x_send_packet(int fd, uint32_t offset, void* obuffer, uint32_t olen)
{
	struct ch36x_ioctl_data ioc_data = {.fd = fd, .offset = offset, .buf = obuffer, .size = olen};
	return ioctl(fd, CH36x_WRITE_PACKET, ioc_data);
}
...
int ch36x_read_config_byte(int fd, uint8_t offset, uint8_t *obyte)
{
	...
	ret = ioctl(fd, CH36x_READ_CONFIG_BYTE,
...
int ch36x_read_config_word(int fd, uint8_t offset, uint16_t *oword)
{
	...
	ret = ioctl(fd, CH36x_READ_CONFIG_WORD,
		    (unsigned long)&ch36x_read_config);
	if (ret < 0) {
		goto exit;
	}
...
int ch36x_read_config_dword(int fd, uint8_t offset, uint32_t *odword)
{
	...
	ret = ioctl(fd, CH36x_READ_CONFIG_DWORD,
		    (unsigned long)&ch36x_read_config);
	if (ret < 0) {
		goto exit;
	}
...
int ch36x_write_config_byte(int fd, uint8_t offset, uint8_t ibyte)
{
	...
	return ioctl(fd, CH36x_WRITE_CONFIG_BYTE,
...
int ch36x_write_config_word(int fd, uint8_t offset, uint16_t iword)
{
	...
	return ioctl(fd, CH36x_WRITE_CONFIG_WORD,
		     (unsigned long)&ch36x_write_config);
}
...
int ch36x_write_config_dword(int fd, uint8_t offset, uint32_t idword)
{
	...
	return ioctl(fd, CH36x_WRITE_CONFIG_DWORD,
		     (unsigned long)&ch36x_write_config);
}
...
int ch36x_read_io_dword(int fd, uint8_t offset, uint32_t *odword)
{
	...
	ret = ioctl(fd, CH36x_READ_IO_DWORD,
		    (unsigned long)&ch36x_read_io);
	if (ret < 0) {
		goto exit;
	}
...
int ch36x_write_io_byte(int fd, uint8_t offset, uint8_t ibyte)
{
	...
	return ioctl(fd, CH36x_WRITE_IO_BYTE,
		     (unsigned long)&ch36x_write_io);
}
...
int ch36x_write_io_word(int fd, uint8_t offset, uint16_t iword)
{
	...
	return ioctl(fd, CH36x_WRITE_IO_WORD,
		     (unsigned long)&ch36x_write_io);
}
...
int ch36x_write_io_dword(int fd, uint8_t offset, uint32_t idword)
{
	...
	return ioctl(fd, CH36x_WRITE_IO_DWORD,
		     (unsigned long)&ch36x_write_io);
}
...
int ch36x_read_mem_byte(int fd, uint16_t offset, uint8_t *obyte)
{
	...
	ret = ioctl(fd, CH36x_READ_MEM_BYTE,
		    (unsigned long)&ch36x_read_mem);
	if (ret < 0) {
		goto exit;
	}
...
int ch36x_read_mem_word(int fd, uint16_t offset, uint16_t *oword)
{
	...
	ret = ioctl(fd, CH36x_READ_MEM_WORD,
		    (unsigned long)&ch36x_read_mem);
	if (ret < 0) {
		goto exit;
	}
...
int ch36x_read_mem_dword(int fd, uint16_t offset, uint32_t *odword)
{
	...
	ret = ioctl(fd, CH36x_READ_MEM_DWORD,
		    (unsigned long)&ch36x_read_mem);
	if (ret < 0) {
		goto exit;
	}
...
int ch36x_write_mem_byte(int fd, uint16_t offset, uint8_t ibyte)
{
	...
	return ioctl(fd, CH36x_WRITE_MEM_BYTE,
		     (unsigned long)&ch36x_write_mem);
}
...
int ch36x_write_mem_word(int fd, uint16_t offset, uint16_t iword)
{
	...
	return ioctl(fd, CH36x_WRITE_MEM_WORD,
		     (unsigned long)&ch36x_write_mem);
}
...
int ch36x_write_mem_dword(int fd, uint16_t offset, uint32_t idword)
{
	...
	return ioctl(fd, CH36x_WRITE_MEM_DWORD,
		     (unsigned long)&ch36x_write_mem);
}
 ...
 * The function returns 0 if successful, others if fails.
 */
int ch36x_read_mem_block(int fd, uint8_t type, uint16_t offset,
			 uint8_t *obuffer, uint32_t len)
{
	...
	ret = ioctl(fd, CH36x_READ_MEM_BLOCK,
		    (unsigned long)ch36x_read_mem);
	if (ret < 0) {
		goto exit;
	}
 ...
 * The function returns 0 if successful, others if fails.
 */
int ch36x_write_mem_block(int fd, uint8_t type, uint16_t offset,
			  uint8_t *ibuffer, uint32_t len)
{
	...
	ret = ioctl(fd, CH36x_WRITE_MEM_BLOCK,
		    (unsigned long)ch36x_write_mem);
 ...
 * The function returns 0 if successful, others if fails.
 */
int ch36x_stream_spi(int fd, uint8_t *ibuffer, uint32_t ilen,
		     uint8_t *obuffer, uint32_t olen)
{
	...
	ret = ioctl(fd, CH36x_STREAM_SPI,
		    (unsigned long)&ch36x_stream_spi);
	if (ret < 0)
		goto exit;
...
int ch36x_flash_erase(int fd, uint32_t addr, uint32_t ilen)
{
	...
	return ioctl(fd, CH36x_FLASH_ERASE,
		     (unsigned long)&ch36x_stream_spi);
}
 ...
 * The function returns 0 if successful, others if fails.
 */
int ch36x_flash_read(int fd, uint32_t addr, uint8_t *obuffer,
		     uint32_t olen)
{
	struct ch36x_stream_spi_t {
		uint32_t addr;
	...
	ch36x_stream_spi =
		malloc(sizeof(struct ch36x_stream_spi_t) + olen);
 ...
 * The function returns 0 if successful, others if fails.
 */
int ch36x_flash_write(int fd, uint32_t addr, uint8_t *ibuffer,
		      uint32_t ilen)
{
	struct ch36x_stream_spi_t {
		uint32_t addr;
	...
	ch36x_stream_spi =
		malloc(sizeof(struct ch36x_stream_spi_t) + ilen);

	ch36x_stream_spi->addr = addr;
	ch36x_stream_spi->ilen = ilen;
	memcpy(ch36x_stream_spi->data, ibuffer, ilen);

	ret = ioctl(fd, CH36x_FLASH_WRITE,
		    (unsigned long)ch36x_stream_spi);
...
int CH36X_SDA_GET(int fd)
{
	...
	ch36x_read_io_byte(fd, offsetof(mCH367_IO_REG, mCH367GPIR),
			   &obyte);
...
void CH36X_SDA_SET(int fd)
{
	...
	ch36x_read_io_byte(fd, offsetof(mCH367_IO_REG, mCH367GPOR),
			   &obyte);
	ch36x_write_io_byte(fd, offsetof(mCH367_IO_REG, mCH367GPOR),
			    obyte | CH367_GPOR_SET_SDA_BIT);
}
 ...
 * The function returns 0 if successful, others if fails.
 */
int ch36x_i2c_write(int fd, uint8_t addr, uint8_t reg, uint8_t *ibuffer,
		    uint32_t ilen)
{
	int i;
	uint8_t obyte;
 ...
 * The function returns 0 if successful, others if fails.
 */
int ch36x_i2c_read(int fd, uint8_t addr, uint8_t reg, uint8_t *obuffer,
		   uint32_t olen)
{
	int i;
	uint8_t obyte;
