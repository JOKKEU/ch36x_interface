...
enum CHIP_TYPE { CHIP_CH365 = 1, CHIP_CH367, CHIP_CH368 };

enum INTMODE { INT_NONE = 0, INT_LOW, INT_HIGH, INT_RISING, INT_FALLING };
...
#define CH365_VID 0x4348
#define CH365_DID 0x5049
#define CH367_VID 0x1C00

#define CH367_DID_SID_HIGH 0x5831 /* Device ID when SID high */
#define CH367_DID_SID_LOW 0x5830 /* Device ID when SID low */
#define CH367_SUB_VID 0x1C00 /* Subsys vendor ID */
#define CH367_SUB_DID_SID_HIGH 0x5831 /* Subsys Device ID when SID high */
#define CH367_SUB_DID_SID_LOW 0x5830 /* Subsys Device ID when SID low */

#define CH368_VID 0x1C00 /* Vendor ID */
#define CH368_DID 0x5834 /* Device ID */
#define CH368_SUB_VID 0x1C00 /* Subsys Vendor ID */
#define CH368_SUB_DID 0x5834 /* Subsys Device ID */
...
#define CH367_SPICR_SPI_FREQ_BIT \
	BIT(5) /* Set SPI Freq: 1->15.6MHz 0->31.3MHz */
#define CH367_SPICR_SPI_SLT_SDI_BIT \
	BIT(6) /* Set SPI Data In Pin: 1->SDI 0->SDX */
#define CH367_SPICR_SPI_NEWTRAN_BIT \
	BIT(7) /* Begin New Transfer After Read SPIDR */

typedef struct _CH365_IO_REG { /* CH365 IO space */
	uint8_t mCh365IoPort[0xf0]; /* 00H-EFH: 240 bytes standard IO bytes */
	union {
		uint16_t mCh365MemAddr; /* F0H: A15-A0 address setting register */
		struct {
			uint8_t mCh365MemAddrL; /* F0H: A7-A0 address setting register */
			uint8_t mCh365MemAddrH; /* F1H: A15-A8 address setting register */
		};
	};
	uint8_t mCh365IoResv2; /* F2H */
	uint8_t mCh365MemData; /* F3H: Memory data access register */
	uint8_t mCh365I2cData; /* F4H: I2C data access register */
	uint8_t mCh365I2cCtrl; /* F5H: I2C control and status register */
	uint8_t mCh365I2cAddr; /* F6H: I2C address setting register */
	uint8_t mCh365I2cDev; /* F7H: I2C device address and command register */
	uint8_t mCh365IoCtrl; /* F8H: high 5 bits are read-only */
...
typedef struct _CH367_IO_REG { /* CH367 IO space */
	...
	union {
		/* ECH: 8-bit passive parallel interface data buffer */
		uint8_t mCH367IoBuf8;
		/* ECH: 32-bit passive parallel interface data buffer */
		uint32_t mCH367IoBuf32;
	};
	union {
		uint16_t mCH368MemAddr; /* F0H: A15-A0 address setting register */
		struct {
			/* F0H: A7-A0 address setting register */
			uint8_t mCH368MemAddrL;
			union {
				/* F0H: A15-A8 address setting register */
				uint8_t mCH368MemAddrH;
				/* F1H: General output register 2 */
				uint8_t mCH367GPOR2;
			};
		} ASR;
	};
	uint8_t mCH367IORESV2; /* F2H */
	uint8_t mCH368MemData; /* F3H: Memory data access register */
	union {
		uint8_t mCH367Data8Sta; /* F4H: D7-D0 port status register */
		uint32_t mCH367SData32Sta; /* F4H: D31-D0 port status register */
	};
	uint8_t mCH367status; /* F8H: Misc control and status register */
	uint8_t mCH367IO_RESV3; /* F9H */
	uint8_t mCH367Speed; /* FAH: Speed control register */
	uint8_t mCH367PDataCtrl; /* FBH: Passive parallel control register */
...
#define mMAX_BUFFER_LENGTH \
	MAX(sizeof(mCH367_IO_REG), sizeof(mCH368_MEM_REG))


#define CH36x_READ_PACKET _IOR(IOCTL_MAGIC, 0xA3, uint16_t)
#define CH36x_WRITE_PACKET _IOW(IOCTL_MAGIC, 0xA4, uint16_t)	
	
struct __attribute__((__packed__)) ch36x_ioctl_data
{
	int fd;
	uint32_t offset;
	uint32_t size;
...
extern int ch36x_send_packet(int fd, uint32_t offset, void* obuffer, uint32_t olen);
extern int ch36x_recv_packet(int fd, uint32_t offset, void* ibuffer, uint32_t ilen);

/**
 * ch36x_open - open ch36x device
 * @devname: the device name to open
 *
 * The function returns the new file descriptor,
 * returns a positive number if successful, negative if fails.
 */
 ...
 * The function returns 0 if successful, others if fails.
 */
extern int ch36x_read_config_dword(int fd, uint8_t offset,
				   uint32_t *odword);
 ...
 * The function returns 0 if successful, others if fails.
 */
extern int ch36x_write_config_dword(int fd, uint8_t offset,
				    uint32_t idword);
 ...
 * The function returns 0 if successful, others if fails.
 */
extern int ch36x_read_mem_block(int fd, uint8_t type, uint16_t offset,
				uint8_t *obuffer, uint32_t len);
 ...
 * The function returns 0 if successful, others if fails.
 */
extern int ch36x_write_mem_block(int fd, uint8_t type, uint16_t offset,
				 uint8_t *ibuffer, uint32_t len);
 ...
 * @fd: file descriptor of ch36x device
 * @mode: bit0: SPI Freq, 0->31.3MHz, 1->15.6MHz
 * 		  bit1: SPI Pinout, 0->SPI3(SCS/SCL/SDX), 1->SPI4(SCS/SCL/SDX/SDI)
 *
 * The function returns 0 if successful, others if fails.
 */
 ...
 * The function returns 0 if successful, others if fails.
 */
extern int ch36x_stream_spi(int fd, uint8_t *ibuffer, uint32_t ilen,
			    uint8_t *obuffer, uint32_t olen);
 ...
 * The function returns 0 if successful, others if fails.
 */
extern int ch36x_flash_read(int fd, uint32_t addr, uint8_t *obuffer,
			    uint32_t olen);
 ...
 * The function returns 0 if successful, others if fails.
 */
extern int ch36x_flash_write(int fd, uint32_t addr, uint8_t *ibuffer,
			     uint32_t ilen);
 ...
 * The function returns 0 if successful, others if fails.
 */
extern int ch36x_i2c_write(int fd, uint8_t addr, uint8_t reg,
			   uint8_t *ibuffer, uint32_t ilen);
 ...
 * The function returns 0 if successful, others if fails.
 */
extern int ch36x_i2c_read(int fd, uint8_t addr, uint8_t reg,
			  uint8_t *obuffer, uint32_t olen);
